        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <!-- 这里在路由跳转之前，还发了请求，把购买产品的信息通过请求的形式通知服务器，服务器进行相应的存储 -->
...
export default {
  ...
  methods: {
    ...
    // 表单元素修改产品个数
    changeNum(event) {
      // 将用户输入的文本隐式转化为Number，*1或者+，非法会转化为NaN
      let value = +event.target.value;
      // 如果用户输入进来的内容非法(不是数字、小于1)，合法不合理(浮点数)
    ...
    async addShopCar() {
      ...
      try {
        await this.$store.dispatch("addOrUpdateShopCar", {
          ...
          skuNum: this.skuNum,
          // 这里派发了action，向服务器发送请求了，但还需要判断请求发送成功还是失败
          // 上面这行代码实际在调用仓库中的addOrUpdateShopCar函数，这个方法带有async,其返回值一定是一个promise，要么成功，要么失败
        });
        // 2.请求发送成功，服务器存储成功：进行路由跳转，并传递参数

        // 下面这种手段进行路由跳转以及传参可行(只是地址栏有点长)
        // this.$router.push({
        //   name: "addcartsuccess",
        //   query: {
        ...
        // 3.存储失败：给用户进行提示（Promise失败的结果写的是一个Error，捕获这个异常）
        alert(error.message);
      }
    },
  },
